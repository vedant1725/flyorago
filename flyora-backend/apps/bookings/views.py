...
class BookingListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            # Determine sender and traveler based on who created the trip
            if trip.airline == 'SENDER_REQUEST':
                # The trip is a Sender Request, so the trip owner is the Sender
                sender_user = trip.user
                traveler_user = request.user
            else:
                # The trip is a Traveler Trip, so the trip owner is the Traveler
                sender_user = request.user
                traveler_user = trip.user
